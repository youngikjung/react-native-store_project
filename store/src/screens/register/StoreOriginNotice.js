...
const StoreOriginNotice = oProps => {
    ...
    const asyncData = async () => {
        ...
        const oResponse = await oProps.appManager.accessAxios("/app/ceo/store/originIntroduction-" + store_id, "get", "text", null);
    ...
    const access = async () => {
        if(introduction === ""){
            ...
            const oResponse = await oProps.appManager.accessAxios("/app/ceo/store/originIntroduction", "post", null, oData);
            if (oResponse !== undefined && oResponse !== null) {
                if(sLocation !== ""){
                    let temp = AppRoute.HOME;
                    if (sLocation === "order"){
        ...
        <TouchableOpacity activeOpacity={1} onPress={Keyboard.dismiss} style={{flex:1, backgroundColor: "#fff"}}>
            ...
            {sLocation !== "" ?
                <View style={{height: height * 0.13, backgroundColor: "#fff", justifyContent: "center", marginLeft: "5%"}}>
                    <Text style={{fontWeight: "600", fontSize: RFPercentage(2.8), color: "#191F28", lineHeight: RFPercentage(3.5) }}>원산지 표시사항을 입력해주세요..</Text>
                </View>
...
export default StoreOriginNotice;