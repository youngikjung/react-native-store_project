...
const SliderScreen = ({ sValue, fnSelect, fnSelected }) => {
    const renderThumb = useCallback(() => {
        return (
            <ComponentSliderToggle iHeight={height * 0.05} iWidth={width * 0.05} />
        )
    }, []);
    ...
    const renderRail = useCallback(() => {
        return (
            <LinearGradient
                ...
                colors={['#C5D5F4', '#0E7BD3','#E32938']} 
                style={{
                    height: height * 0.01, backgroundColor: "#6490E7", width : width * 0.85, borderRadius: 20
                }}
            />
        )
    }, []);
    ...
    const renderRailSelected = useCallback(() => {
        return (
            <View style={{height: height * 0.01, backgroundColor: "#001E62", borderRadius: 20}} />
        )
    }, []);
    ...
    const renderLabel = useCallback(value => {
        return (
            <>
                <View style={{ zIndex: 3, width: width * 0.12, backgroundColor: "#0E7BD3", height: height * 0.04, borderRadius: width * 0.02, justifyContent: "center", alignItems: "center", marginBottom: "5%"}}>
                    <Text style={{color: "#fff", fontWeight: "800", fontSize: RFPercentage(1.7) }}>{value} 분</Text>
                </View>
                <View style={{position: "absolute", zIndex: 2, bottom: height * 0.015, left: "30%", height: height * 0.015, width: height * 0.015, backgroundColor: "#0E7BD3", transform: [{ rotateZ: '0.785398rad' }]}} />
            </>
        )
    }, []);
