...
const List = ({ fnGoBack, sProps, iRef }) => {
    ...
    const checkNotice = () => {
        ...
        if(process1 && process2 && process3){
            ...
            if(sCouponId !== undefined && sCouponId !== null){
                sProps.appManager.showModal(
                    true,
                    <ComConfirmStamp 
                        sTitle={"edit"}
    ...
    useEffect(() => {
        if(iRef !== undefined && iRef !== null){
            ...
            setNm(iRef.name);
            setId(iRef.id);
            setCouponId(iRef.couponId);
            setMinAmount(iRef.couponCount.toString());
            setCouponAmount(iRef.amount.toString());
            setPeriod(iRef.period);
            setPeriodNm(iRef.periodNm);
            setTarget(iRef.target);
            setTargetNm(`총 ${iRef.target}개`);
            setTextInputType("");
        }
    }, [iRef]);
                ...
                <>
                    ...
                    {textInputType === "" &&
                        <View style={{height: height / 6 , backgroundColor: "#fff", justifyContent: "center"}}>
                            ...
                            >
                                <Text style={{ fontSize: RFPercentage(2), fontWeight: '800', color: '#fff' }}>스템프 수정하기</Text>
                            </TouchableOpacity>
                        </View>
                    }
                </>
